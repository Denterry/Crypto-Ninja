...
var scam = preload("res://Scenes/FRAX.tscn")

var ripple_scam = preload("res://Scenes/RIPPLE(scam).tscn")
...
func _on_generator_timeout() -> void:
	...
	for i in range(0, randi_range(1, 5)):
		...
		var token_instance
		match type:
			0: token_instance = lido.instantiate()
			1: token_instance = lido.instantiate()
			2: token_instance = lido.instantiate()
		...
		token_instance.life.connect(self.dec_life)

		if type != 8:
			token_instance.score.connect(self.inc_score)
